...
const Homefruits = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3012/table-data1`)
      .then(response => {
  ...
  const deleteFruits = (id) => {
    ...
    if (confirmDelete) {
      axios.delete(`http://localhost:3012/delete1/${id}`).then((response) => {
        setData(
          data.filter((val) => {
            return val.id !== id;
          })
  ...
  const updateFruit = async (id, newQuantite, newQuantiteVendu) => {
    try {
      const response = await axios.put(`http://localhost:3012/UpdateQuantities2/${id}`, {
  ...
  const handleSubmitVendu = () => {
    if (selectedProduct) {
      ...
      updateFruit(selectedProduct.id, newQuantite, newQuantiteVendu);
...
<div style={{marginBottom:'5px',}}>
      <h2 style={{display:'flex', justifyContent:'right'}}>
        <Link to="/addfruits" className="linkStyle">
          + AJOUTER PRODUITS
        </Link>
      </h2>
      <input
        ...
        <tbody>
          {filteredData.map((item, index) => (
            <tr key={index}>
              ...
              <td>{item.prix_achat} dh/kg</td>
              <td>{item.prix_vente} dh/kg</td>
              <td>{item.gain} dhs</td>

              <td className="actions" style={{textAlign:'center'}}>
                <button className='BT-UPDATE' onClick={() => {
                        navigate(`/Updatefruits/${item.id}`); 
                      }}>Modifier</button>
                ...
                <button className='BT-SUPPRIMER' onClick={() => deleteFruits(item.id)}>Supprimer</button>
              </td>
            </tr>
          ))}
        </tbody>
