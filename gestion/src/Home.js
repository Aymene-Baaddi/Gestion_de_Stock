...
const Home = () => {
  ...
  useEffect(() => {
    axios.get(`http://localhost:3012/table-data`)
      .then(response => {
  ...
  const deleteProduits = (id) => {
    ...
    if (confirmDelete) {
      axios.delete(`http://localhost:3012/delete/${id}`).then((response) => {
        setData(
          data.filter((val) => {
            return val.id !== id;
          })
  ...
  const updateProduct = async (id, newQuantite, newQuantiteVendu) => {
    try {
      const response = await axios.put(`http://localhost:3012/UpdateQuantities/${id}`, {
  ...
  return (
    <div className="table-container">
      ...
      <h2 style={{display:'flex', justifyContent:'right'}}>
        <Link to="/addProduits" className="linkStyle">
          + AJOUTER PRODUITS
        </Link>
      </h2>
      <input
